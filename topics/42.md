...
> <span data-guild-class="callout tip">Tip</span> Use the `--print-cmd` option with [`guild run`](/commands/run) to print the full command Guild uses when running an operation. This command includes the flag related options as described above.
...
> <span data-guild-class="callout tip">Tip</span> Use environment variables from non-Python scripts as a convenient way to read flag values without having to process command line arguments.

### Python Global Variables

Guild sets flag values as global variables in a Python script when `flags-dest` is set to `globals` or when Guild otherwise [detects this interface through inspection](/docs/defaults.md#flags-interface). (TODO: this link is to the wrong topic)
...
> <span data-guild-class="callout tip">Tip</span> When `flags-import` is `no`, Guild will not inspect your script for flags. Use this value to avoid processing your scripts when you don't need to.
