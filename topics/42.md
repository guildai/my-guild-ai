...
> <span data-guild-class="callout tip"></span>Use the `--print-cmd` option with [`guild run`](/commands/run) to print the full command Guild uses when running an operation. This command includes the flag related options as described above.
...
> <span data-guild-class="callout tip"></span>Use environment variables from non-Python scripts as a convenient way to read flag values without having to process command line arguments.
...
To import flags, Guild inspects a script and look for possible flag definitions based on the [flags interface](#flags-interface). As described above, the flags interface may be explicitly configured. Otherwise Guild attempts to infer the interface.
...
> <span data-guild-class="callout tip"></span>When `flags-import` is `no`, Guild will not inspect your script for flags. Use this value to avoid processing your scripts when you don't need to.

## Flag Definitions

TODO (see [flags reference](/reference/guildfile#flags))
...
Value lists are specified in the format <code>[<em>VAL1</em>,<em>VAL2</em>,...]</code> where each value is a number, a string, or boolean value.
