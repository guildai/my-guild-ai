...
The term *workflow* in this section refers to running multiple [operations](ref:operation) to accomplish a goal.

Guild workflows are implemented using higher-level operations, which define sub-operations using a `steps` attribute.
...
When you want to automate a sequence of operations, you can create a higher-order operation.
...
When you run `end-to-end`, Guild starts a higher-order operation, which runs each of the specified steps in the order specified.
...
To expose a step flag value to the user, you must define a flag in the higher order operation and explicitly use it as a step flag.
