...
To add Guild support for your project, add a [Guild file](/docs/guildfile). In most cases, you don't need to change the code. Simply add a file to enable experiment tracking and other features.

> <span data-guild-class="callout note">Note</span> This guide applies to Python based projects. If your project uses a different language, refer to [*Languages Example*](/examples/languages). If you don't see a suitable example for the language, [ask for help](/new-topic?category=general).
...
Remember to activate the environment. When you open a new terminal, activate the environment in that terminal as well.
...
In the activated environment, ensure that the environment uses the latest version of `pip`:
...
Before using Guild, run the operation directly with Python. This step resolves project related issues before introducing Guild.
...
> *Configure the operations to take less than 10 seconds.*
...
With this change, the operation runs within a few second --- about as long as it takes to load the program and read the MNIST examples.

Why bother with a 10 second rule? If you're skeptical of this idea, skip it for now. But pay attention to the time you spend waiting for non-essential computation. Notice what happens to your attention after even 20 seconds of waiting. Many of us are tempted to use this downtime for productive work like checking news or browsing r/MachineLearning. If you feel these distractions slow your development, return to this step and try the suggestions.
...
From the project directory, use [`guild run`](/commands/run) with the `--test-sourcecode` option:

``` command
guild run --test-sourcecode
```

> <span data-guild-class="callout note">Note</span> If the Guild file only contains one operation, you don't need to specify the operation name when running [`guild run`](/commands/run). With more than one operation, you need to specify the operation name or designate one of the operations as default using the [`default`](/reference/guildfile#operation-default) attribute.
...
If there are missing source code files, the operation won't run correctly. Guild isolates runs from project source code. This prevents changes to project source code from affecting the operation run.

Guild uses default rules to detect likely source code files. Guild uses safeguards to avoid copying too many files or files that are too big. If Guild excludes files due to safe guards, it logs a warning message.

If you need to, change Guild's default source code copy rules by defining the [`sourcecode`](/reference/guildfile#operation-sourcecode) operation attribute.

Reasons for defining operation `sourcecode` include:
...
1. Create a new run directory in the `runs` subdirectory of [Guild home](/docs/environment#guild-home)
...
If the operation fails, don't panic!

First, confirm that you can run the operation using Python directly. See [Run Without Guild](#run-without-guild) above for help. Don't troubleshoot issues with Guild until you know the operation runs as expected without Guild.
...
Refer to the steps below for help resolving these problems.

## Resolve Missing Source Code

If the operation is missing source code, the error message typically contains `ImportError: No module named ...` or `ModuleNotFoundError: No module named ...` depending on the Python version. Diagnose this problem by listing the source code files copied for the run with [`guild ls`](/commands/ls):

``` command
guild ls --sourcecode
```

The `--sourcecode` option tells Guild to list source code files.

Confirm that the module from the error message is missing. Adjust the `sourcecode` operation attribute as described in [Test Operation Source Code](#test-operation-source-code).

When you're ready, re-run the operation.

## Resolve Missing Input Files

If the operation is missing required input files, the error message often contains `IOError: [Errno 2] No such file or directory: ...` or `FileNotFoundError: [Errno 2] No such file or directory: ...` depending on the Python version. The message may differ but generally refers to a missing file.

*This is the most common issue when running Guild operations and is easy solved.*
...
> <span data-guild-class="callout important">Important - Read Twice</span>
...
It reads a file named `config.json` located in the current directory. When you run it with Python from the directory containing `config.json`, it works file. When you run it with Guild, it fails with the message `FileNotFoundError: [Errno 2] No such file or directory: 'config.json'`.

When the module is run with Guild, it runs in a newly created, empty directory. `config.json` isn't there!

Check the files in a run directory using [`guild ls`](/docs/ls):
...
This is an important design feature: *Guild starts with empty directories to ensure that required files are explicitly defined in the Guild file.*

In the case above, we make `config.json` available by modifying the Guild file:
...
By default, Guild creates [*symbolic links*](https://en.wikipedia.org/wiki/Symbolic_link) to resolved files rather than copies. In version 0.7 you have the option to create a file *copy*. This is an important consideration for auditing and reproducibility. Refer to [*File Dependencies*](/docs/dependencies#files) for more information.
...
``` python
saved_model_path = "output/model.mdf5"
...
``` yaml
train:
  main: my_mod --saved-model model.mdf5
```
...
### Hard-Coded Hyperparameters
 ...
 Hard-coded hyperparameters are important values that ought to be configurable but aren't. Here's an example from [`mnist_mlp.py`](https://github.com/guildai/guildai/blob/master/examples/get-started/mnist_mlp.py#L36-L41):
...
Implicit hyperparameters are values that are not specified in your code but are defined as defaults in libraries. Consider this example, once again from [`mnist_mlp.py`](https://github.com/guildai/guildai/blob/master/examples/get-started/mnist_mlp.py#L45-L47):
...
https://github.com/guildai/guildai/blob/master/examples/get-started/mnist_mlp_args.py#L22
</div>
...
[`mnist_mlp_config.py`](https://github.com/guildai/guildai/blob/master/examples/get-started/mnist_mlp_config.py) is a modified version of `mnist_mlp.py` that reads settings from `config.json`.

<div data-guild-github-select="18-22">

https://github.com/guildai/guildai/blob/master/examples/get-started/mnist_mlp_config.py#L20
</div>
...
https://github.com/guildai/guildai/blob/master/examples/get-started/config.json
