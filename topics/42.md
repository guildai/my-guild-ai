...
Flags are specified for an operation using ``NAME=VALUE`` arguments to the [`run`](/commands/run) command.
...
!!! tip Use the `--print-cmd` option with [`run`](/commands/run) to print the full command Guild uses when running an operation. This command includes the flag related options as described above.
...
!!! tip Use environment variables from non-Python scripts as a convenient way to read flag values without having to process command line arguments.
...
!!! tip When `flags-import` is ``no``, Guild will not inspect your script for flags. Use this value to avoid processing your scripts when you don't need to.
...
^ Implicitly import each defined flags --- use `flags-import` or `flags-import-skip` to control this
